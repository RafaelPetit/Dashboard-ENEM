...
# CSS compartilhado entre tooltip de título e métricas
_TOOLTIP_CSS = """
<style>
.tooltip-container {
    position: relative;
...
def _injetar_css_tooltip():
    """Injeta o CSS de tooltips uma única vez por sessão."""
    if '_tooltip_css_injetado' not in st.session_state:
        st.markdown(_TOOLTIP_CSS, unsafe_allow_html=True)
        st.session_state._tooltip_css_injetado = True
